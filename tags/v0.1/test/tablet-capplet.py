...
############################################################################
##
## Copyright (C) 2007 Alexander Macdonald, Juho Vepsalainen. All rights reserved.
##
...
def GetID(devicename):
	try:
		output = subprocess.Popen(["xsetwacom", "get", devicename, "ToolID"], stdout=subprocess.PIPE).communicate()[0]
		return int(output.strip())
	except:
		return None
...
class GraphicsTabletApplet:
	def __init__(self, gladefile):
		...
		self.DeviceID = self.WidgetTree.get_widget("deviceID")
	...
	def GetWheel(self):
		dev = gtk.gdk.devices_list()[self.Device]
		state = dev.get_state(self.DrawingArea.window)
		return dev.get_axis(state[0], gtk.gdk.AXIS_WHEEL)
	...
	def Update(self):
		...
		id = GetID(self.DeviceName)
		
		if id == None:
			id = ""
		
		self.DeviceID.set_label(str(id))
	
		return True

################################################################################

a = GraphicsTabletApplet("tablet-capplet.glade")
a.Run()
